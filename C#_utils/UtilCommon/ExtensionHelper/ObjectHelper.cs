...
namespace Util.Common.ExtensionHelper
{
    ...
    public static class ObjectHelper
    {
        ...
        {
            //t.GetType().Assembly.CreateInstance(t.GetType().Name);
            object result = t.GetType().Assembly.CreateInstance(t.GetType().FullName);
        ...
        public static void CopyTo(this Object res, Object des, List<string> properties = null)
        {
            List<PropertyInfo> resourcePropertys = res.GetType().GetProperties().ToList();
            foreach (PropertyInfo desProperty in des.GetType().GetProperties())
            {
                string name = desProperty.Name;
                if (properties != null && properties.Count(p => p == name) == 0)
                {
                    continue;
                }
                if (false == desProperty.CanWrite)
                {
                    continue;
                }
                PropertyInfo resourceProperty = resourcePropertys.FirstOrDefault(p => p.Name == name);
                if (resourceProperty == null)
                {
                    continue;
                }
                if (false == resourceProperty.CanRead)
                {
                    continue;
                }
                object value = resourceProperty.GetValue(res, null);
                if (value == null)
                {
                    continue;
                }
                switch (desProperty.PropertyType.Name)
                {
                    case "Byte":
                    case "Int16":
                    case "Int32":
                    ...
                    case "Decimal":
                    case "String":
                        desProperty.SetValue(des, value, null);
                        break;
                    default:
                        Console.WriteLine(desProperty.PropertyType.Name);
                        if (desProperty.PropertyType.Name.EndsWith("[]"))
                        {
                            desProperty.SetValue(des, value, null);
                        }
                        else
                        {
                            object resourceValue = resourceProperty.PropertyType.Assembly.CreateInstance(resourceProperty.PropertyType.FullName);
                            value.CopyTo(resourceValue);
                            desProperty.SetValue(des, resourceValue, null);
                        }
                        break;
                }
            }
        ...
        /// <param name="sb">StringBuilder</param>
        /// <param name="obj">追加对象</param>
        public static void Appendln(this StringBuilder sb, object obj)
        {
            sb.Append(obj);
            sb.AppendLine();
        }
        ...
        /// <param name="args">参数</param>
        public static void AppendFormatln(this StringBuilder sb, string format, params object[] args)
        {
